...
function calcMotion(tweets: Tweet[], followerCount: number): number {
  const totalWeighted = tweets.reduce((sum, t) => {
    return sum + t.bookmarkCount * 3 + t.replyCount * 2 + t.likeCount * 1;
  }, 0);
...
function calcVolume(tweets: Tweet[]): number {
  ...
  const tweetsPerWeek = 10 / (days / 7);
  if (tweetsPerWeek < 1) return Math.round(tweetsPerWeek * 30);
  if (tweetsPerWeek <= 10) return Math.round(30 + (tweetsPerWeek / 10) * 70);
  return Math.max(Math.round(100 - (tweetsPerWeek - 10) * 5), 20);
}

export function calculateScore(profile: UserProfile, guardian: Guardian | null): ScoreResult {
  ...
  const raw = motion * 0.5 + conviction * 0.3 + volume * 0.2;
